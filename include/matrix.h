...
// コンストラクタ
template <class T> matrix<T>::matrix(int i, int j, const T *s) {
  size_t size = i * j + 1;
...
//  行列sのコピーコンストラクタ
template <class T> matrix<T>::matrix(const matrix<T> &s) {
...
// デストラクタ
template <class T> matrix<T>::~matrix() { delete[] aij; }
...
// 行列sと等しい時に真を返す
template <class T> int matrix<T>::operator==(const matrix<T> &s) const {
  ...
  for (int i = 0; i < s1.ii; i++)
    for (int j = 0; j < s2.jj; j++)
      for (int k = 0; k < s1.jj; k++)
        d.aij[i * d.jj + j] += s1.aij[i * s1.jj + k] * s2.aij[k * s2.jj + j];
...
// 配列dataを代入する
template <class T> matrix<T> &matrix<T>::operator<<=(const T *data) {
  if (!ii || !jj)
...
template <class T> matrix<T> &matrix<T>::operator*=(const matrix<T> &s) {
  ...
  for (int i = 0; i < ii; i++)
    for (int j = 0; j < s.jj; j++)
      for (int k = 0; k < jj; k++)
        d.aij[i * d.jj + j] += aij[i * jj + k] * s.aij[k * s.jj + j];
...
// i行をaで割る
template <class T> void matrix<T>::divi(int i, T a) {
  for (int j = 0; j < jj; j++)
    aij[i * jj + j] /= a;
}

// i1行からi2行をa倍したものを引く
template <class T> void matrix<T>::subi(int i1, int i2, T a) {
  for (int j = 0; j < jj; j++)
    aij[i1 * jj + j] -= aij[i2 * jj + j] * a;
}

// i1行とi2行とを入れ替える
template <class T> void matrix<T>::swapi(int i1, int i2) {
  for (int j = 0; j < jj; j++) {
    T t;
...
// 2次元行列の大きさを知る
template <class T> void matrix<T>::msize(int *i1, int *i2) const {
  *i1 = ii;
  *i2 = jj;
}

// 行列のトレースを求める | 矩阵的迹
template <class T> T matrix<T>::tr(void) const {
  if (!ii || ii != jj)
    err("tr (%d, %d)\n", ii, jj, 0, 0);
...
// 计算行列式
template <class T> T matrix<T>::det(void) const {
  if (!ii || ii != jj)
    err("det (%d, %d)\n", ii, jj, 0, 0);
...
// rankを求める
template <class T> int matrix<T>::rank(void) const {
  if (!ii || !jj)
    err("rank (%d, %d)\n", ii, jj, 0, 0);
...
// dj列を削除した行列を返す
template <class T> matrix<T> matrix<T>::dj(int dj) const {
  matrix<T> d(ii, jj - 1);
...
// di行dj列を削除した行列を返す
template <class T> matrix<T> matrix<T>::dij(int di, int dj) const {
...
template <class T> matrix<T> matrix<T>::inverse(void) const {
  if (!ii || ii != jj)
    err("inverse (%d, %d)\n", ii, jj, 0, 0);
...
// 累乗法により実対称行列の固有値と固有ベクトルを求める
template <class T> matrix<T> matrix<T>::powEig(matrix<T> &c) const {
...
template <class T> inline T &matrix<T>::operator()(int mi, int mj) {
  if (ii < mi - 1 || jj < mj - 1)
    err("operator () (const matrix<T> &s) (%d, %d), (%d, %d)\n", ii, jj, mi,
        mj);
...
template <class T> inline T &matrix<T>::elm(int mi, int mj) {
  return aij[(mi - 1) * jj + mj - 1];
}
...
// 行列のある部分に小行列sを代入する (先頭要素は (1,1))
template <class T> void matrix<T>::pasg(int mi, int mj, const matrix<T> &s) {
...
// 行列sのある部分を小行列として得る (先頭要素は (1,1))
template <class T> matrix<int> matrix<T>::size(void) const {
  matrix<int> d(2, 1);
  d(1, 1) = ii;
  d(2, 1) = jj;
  return d;
...
// 行列が正則(0)か特異(1)か判定する
template <class T> int matrix<T>::snglr(T limit) const {
  if (!ii || ii != jj)
    err("det (%d, %d)\n", ii, jj, 0, 0);
...
template <class T> matrix<T> matrix<T>::normal(void) const {
  if (!ii || jj != 1)
    err("normal (%d, %d)\n", ii, jj, 0, 0);
